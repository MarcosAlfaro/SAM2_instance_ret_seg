...
def build_prompt_inputs(frame_gt: torch.Tensor, prompt: str, training: bool, device: torch.device) -> Dict[str, torch.Tensor]:
    ...
    n_boxes = len(boxes)
    # Convert list of lists to tensor before passing to einops
    boxes_tensor = torch.tensor(boxes, dtype=torch.float32, device=device)
    box_coords = einops.rearrange(boxes_tensor, 'n (p1 p2) -> (n p1) p2', p1=2, p2=2).unsqueeze(0)  # [1, 2n, 2]
...
def get_bounding_boxes(mask):
    ...
    # Ensure mask_np is a standard numpy array to avoid version conflicts
    # Force copy and C-contiguous layout
    # mask_np = np.ascontiguousarray(mask_np, dtype=np.int32)
    # labeled_mask, num_features = label(mask_np)
    
    # Manual bounding box extraction to bypass scipy.ndimage.label issue
    ...
    # Skip the rest of the function that uses label()
    return boxes
