...
import utils
from visualization_utils import bbox_from_mask
...
class BURSTVideo:
    ...
    def filter_dataset_for_benchmark(self, masks_per_frame):
        ...
        if not tracks_count or all(count == 0 for count in tracks_count.values()):
            raise ValueError("No valid tracks found - no frames contain multiple instances of the same object")
