...
from models.sam2.modeling.sam2_base_as import SAM2Base 
from models.sansa.model_utils_as import BackboneOutput, DecoderOutput
from util.path_utils import SAM2_PATHS_CONFIG, SAM2_WEIGHTS_URL
from util.promptable_utils import rescale_prompt
import time

print("DEBUG: sansa_as.py module loaded, using sam2_base_as.SAM2Base")
...
class SANSA(nn.Module):
    def __init__(self, sam: SAM2Base, device: torch.device):
        ...
        self.sam = sam
        self.device = device
        print(f"DEBUG: SANSA_AS initialized with sam type: {type(sam).__name__}")
        ...
        for b in range(B):
            ...
            for idx in range(T):
                ...
                if idx < n_shots:
                    ...
                    if prompt_type == 'mask':
                        print(f"DEBUG: Calling self.sam._use_mask_as_output, self.sam type: {type(self.sam)}, module: {type(self.sam).__module__}")
                    ...
                    fixed_memory_idx = 1
                    decoder_out: DecoderOutput = self._compute_decoder_out_w_mem(backbone_output, absolute_idx, fixed_memory_idx, self.memory_bank)
                if idx < n_shots:
                    mem_entry = self._compute_memory_bank_dict(decoder_out, backbone_output, absolute_idx)
                    if idx == 0:
                        self.memory_bank[0] = mem_entry
                
                # Append all masks (should be [3, 1, H, W] when multimask_output=True)
                print(f"DEBUG: Frame {idx}, decoder_out.masks shape: {decoder_out.masks.shape if decoder_out.masks is not None else 'None'}")
                if hasattr(decoder_out, 'low_res_multimasks') and decoder_out.low_res_multimasks is not None:
                    print(f"DEBUG: Frame {idx}, decoder_out.low_res_multimasks shape: {decoder_out.low_res_multimasks.shape}")
                outputs["masks"].append(decoder_out.masks)

        print(f"DEBUG: Total masks collected: {len(outputs['masks'])}")
        print(f"DEBUG: Mask shapes before cat: {[m.shape for m in outputs['masks']]}")
        masks = torch.cat(outputs["masks"])
        print(f"DEBUG: masks shape after cat: {masks.shape}")
...
def build_sansa(sam2_version: str = 'large', adaptformer_stages: List[int] = [2, 3], channel_factor: float = 0.3, device: str = 'cuda',
    ...
    with initialize(version_base=None, config_path=".", job_name="test_app"):
        ...
        cfg.model.multimask_output_for_tracking = True
