...
class SANSA(nn.Module):
    ...
    def forward(self, samples: torch.Tensor, prompt_dict: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        masks = torch.cat(outputs["masks"])  # [T, 3, 256, 256]
        # Interpolate to original size - masks is already [T, C, H, W] format
        masks = F.interpolate(masks, size=orig_size[0], mode='bilinear', align_corners=False)

        if len(outputs["object_score_logits"]) > 0:
            object_score_logits = torch.cat(outputs["object_score_logits"], dim=0) 
