...
def convert_mask_values(mask):
        ...
        # Ensure it's a standard numpy array of floats before thresholding
    ...
    def forward(self, samples: torch.Tensor, prompt_dict: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for b in range(B):
            ...
            for idx in range(T):
                ...
                # Ensure we only have one mask per frame (the best one)
                ...
                if decoder_out.masks.shape[1] > 1:
                    ...
                    best_iou_inds = torch.argmax(decoder_out.ious, dim=-1)
                    batch_inds = torch.arange(B, device=decoder_out.masks.device)
                    decoder_out.masks = decoder_out.masks[batch_inds, best_iou_inds].unsqueeze(1)
        ...
        return {"pred_masks": masks, "object_score_logits": object_score_logits, "obj_pointers": obj_pointers}
